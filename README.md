...
- **审计日志**：脚本每次联网运行把完整原始响应与解析结果追加到 `data/quotes.jsonl`（已 gitignore，仅服务器本地）——任何推送数值事后可回查对账；**推送时点在日志中无对应记录即为编造实锤**。单文件超 20MB 自动轮转为 `quotes.jsonl.1`（覆盖式只保留一代，`audit` 汇总两代合并读取，总量上限约 40MB，无需外部 logrotate）。周度复盘内置 audit 对账节，逐周把脚本运行记录与 hermes 实际推送并列陈列。
...
- **行情对账（幻觉排查）**：任何推送数值存疑时，第一步入服务器查审计日志：`grep 'MM:SS' ~/stock-monitor/data/quotes.jsonl`（或按日期过滤 `python3 ~/stock-monitor/scripts/quote.py audit --this-week`）对照该时点的原始接口响应——推送时点无对应记录即为编造实锤，有记录则以记录为准核对每个数值。
