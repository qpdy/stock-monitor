...
# ---------------------------------------------------------------- selftest（离线）
#
# 注：本段大量 fixture 仍沿用 sz000582 / 北部湾港 作为示例 symbol 与 stock_name。
# 北部湾港已不在监控配置中（schedule.json 2026-09-16 撤销），但 selftest 测的是
# 脚本通用解析/涨停/停牌/自洽/对账/复读补检等逻辑——与具体监控标的无关；每条
# 断言均可追溯到 2026-08-27 竞价初始任务编造自洽假行情事故的回归 case。删 fixture
# 会损失离线回归覆盖与事故溯源语境，无功能收益。股票监控改动无需同步改本段。
